...
Phase 0  intake 清单 → 确认卡 → 写入画像/模态
   ...
   Step 3.5 校验自带阅读器：start.command → npm test → verify_reader.js
   闸 B     模板清理（可推迟；绝不阻塞学习）
        ↓
...
4. 说「排期」→ 确认提案 → 「开始生成」。  
5. 用自带的通用阅读器阅读。Step 3.5 是校验而非构建：`start.command` 启动它，`npm test` 覆盖锚定规则与服务器路径安全，`node scripts/verify_reader.js` 检查完整契约（主题、偏好记忆、排序、Notes 跳转、context 定位、多文档隔离、无 Git UI）。  
6. 「帮我批改」；加练须先被询问并同意。
...
| 项 | 原因 |
| :--- | :--- |
| 发布**成品**阅读器 | macOS 分支已完成：仅本地运行、文件接口收窄的通用阅读器，现已符合锁定契约。`templates/reader_skeleton.html` 作为参考外壳保留，供其它分支移植 UI |
...
- [x] macOS 分支自带通用阅读器（`index.html` / `server.js` / Mermaid / Notes / 自动保存）
- [x] 阅读器符合锁定契约：`html[data-theme]` + FOUC 守卫、四个记忆偏好、全标记 + `data-primary` 锚定、无 Git UI
- [x] 验收脚本 `scripts/verify_reader.js`（50 项检查）与 `ReaderCore.annotationMatches` 单元测试  
