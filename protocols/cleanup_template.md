...
6. **保留已确认的个人偏好**：
   * 删除 Phase 0 前期访谈 prompt 时，禁止删除 `knowledge/profile.md` 中已经确认的值，包括主要解释语言、学习内容语言、无障碍需求、时间约束和学习偏好。
...
4. **下一步行动指引**：提示用户双击 `start.command` 启动浏览器，并说「今天学什么」或「排期」正式进入 Phase 1。
