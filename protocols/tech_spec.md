...
### 3.5 多选题（MSQ）— 正确答案 ≥2 项

```markdown
#### MSQ-1
Stem：下列关于 X 的描述中，哪些正确？（多选）
- [ ] A. 选项
- [ ] B. 选项
- [ ] C. 选项
- [ ] D. 选项
<!-- answers: A,C | rationale: 一句话依据，指向正文概念 Y -->
```

规则：

- 题号前缀必须是 `MSQ-`，与单选的 `MCQ-` 区分；题干必须写「（多选）」，否则用户会当单选做。
- 答案注释用**复数** `answers:`；字母用英文逗号分隔、升序、逗号后无空格：`A,C`。
  单数 `answer:` 只属于 MCQ 与 T/F —— 写错前缀 `validate_content.js` 会报缺答案。
- 正确项必须 **≥2**；只有一个正确项时改用 MCQ。
- 选项 4–6 个；用户把所有选中的项改成 `- [x]`，其余留 `[ ]`。
- 干扰项要「部分正确但越界」，考的是边界而不是记忆。
- 判分口径（`p3_review.md` §2.1）：全对满分；**漏选**按比例给部分分；**多选到错误项**该题 0 分。
