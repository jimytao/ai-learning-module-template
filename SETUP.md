...
**本分支面向 macOS，启动脚本是 `start.command`。** 请检查：

1. 根目录确实存在 `start.command`，且有可执行权限（`ls -l start.command` 应看到 `x`；
   丢失时用 `chmod +x start.command` 恢复 —— 复制文件夹后经常会丢）；
2. 它内部引用的端口与 `server.js` 的 `DEFAULT_PORT` 一致；
3. 如果用户实际在 Windows 上，说明他们拿错了分支，建议改用 `Chinese`
   分支，或由你补一个等效的 `start.bat`。
...
然后启动阅读器（双击 `start.command`，或 `npm start` 后自行打开
...
| 初始弱项 | 3–5 条 |
| 环境 | Node 版本 · `npm install` · `start.command` 已验证 |
